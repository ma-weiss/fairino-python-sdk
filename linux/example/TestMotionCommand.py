...
def movec(self):
    ...
    error = robot.MoveC(
        desc_pos_p=DP3, tool_p=0, user_p=0, desc_pos_t=DP1, tool_t=0, user_t=0
    )
...
def circle(self):
    """机器人Circle测试"""
    error = robot.Circle(
        desc_pos_p=DP3, tool_p=0, user_p=0, desc_pos_t=DP2, tool_t=0, user_t=0
    )
...
def newspiral(self):
    """机器人NewSpiral测试"""
    error = robot.NewSpiral(
        desc_pos=DP2_h, tool=0, user=0, param=[5.0, 10, 30, 10, 5, 0]
    )
...
def servocart(self):
    ...
    while i < 200:
        # pos[2] += 0.01
        time.sleep(0.008)
        error = robot.ServoCart(mode=1, desc_pos=pos, exaxis=[0, 0, 0, 0])
        i += 1
    robot.ServoMoveEnd()
...
def jointoverspeedprotect(self):
    """机器人超速保护测试"""
    error = robot.MoveL(
        DP1, tool=0, vel=100, user=0, overSpeedStrategy=3, speedPercent=100
    )
